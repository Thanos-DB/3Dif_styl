...
#     """
#     x: field of view scale factor, small x-> (wider view, zoomed-in)
#     n: near clipping plane, objects closer than this distance will not be visible
#     f: far clipping plane, objectes beyond this distance are clipped out
#     """
...
#
#
def projection(x=0.1, n=1.0, f=50.0, orthographic=False):
    if orthographic:
        # Orthographic projection
        scale = 1/x  # x now acts as a zoom factor: larger x = more zoomed out
        return np.array([[-scale,     0,         0,           0],
                        [     0, -scale,         0,           0],
                        [     0,     0,   2/(f-n),  -(f+n)/(f-n)],
                        [     0,     0,         0,           1]]).astype(np.float32)
    else:
        # Perspective projection (original)
        return np.array([[-n/x,    0,            0,              0],
                         [  0,  n/x,            0,              0],
                         [  0,    0, -(f+n)/(f-n), -(2*f*n)/(f-n)],
                         [  0,    0,           -1,              0]]).astype(np.float32)
