...
def setup_debug():
    """Call this function only when you want to see debug messages"""
...
def setup_normal():
    """Call this function to reset to normal logging (WARNING and above only)"""
    # Reset any existing handlers
    for handler in lg.root.handlers[:]:
        lg.root.removeHandler(handler)
    # Set up logging
...
# By default, use normal logging (WARNING and above only)
setup_normal()
# When you need debug info, just call:
# setup_debug()
#
# # Normal state - only shows warnings and above
# lg.debug("This won't show")
# lg.info("This won't show")
# lg.warning("This will show")
# # # When you need debug info
# # setup_debug()
# # lg.debug("Now this will show")
# # lg.info("This too")
# # lg.warning("And this")
# # # Go back to normal
# # setup_normal()
# # lg.debug("This won't show again")
# # lg.warning("But this will")
...
# os.chdir("/home/thanos/OneDrive/mybackupOneDrive/PhD/3dif_objaverse-xl-subset/earth/")
...
def img_spot():
    ...
    # pos, uv, tri_pos, tri_uv = load_obj("main.obj")
    pos = np.concatenate([pos, np.ones_like(pos[:, :1])], axis=1)
    nrm = np.concatenate([nrm, np.zeros_like(nrm[:, :1])], axis=1)
    #
    # print("vertices:\t", pos.shape)
    # print("texcoords:\t", uv.shape)
    # print("faces:\t\t", tri_pos.shape)
    # print("tfaces:\t\t", tri_uv.shape)
    #
    # tri_pos = tri_pos.astype(np.int32)
    # tri_uv = tri_uv.astype(np.int32)
    # tri_nrm = tri_nrm.astype(np.int32)
 
    
    tex = imageio.v2.imread("default.png")[::-1].astype(np.float32) / 255.0
    # tex = imageio.v2.imread("uv.png")[::-1].astype(np.float32) / 255.0
    # tex = tex[::4,::4,:]
    ...
    # x, y, z
    # translate x: larger -> left
    # translate y: larger -> down
    # translate z: larger -> zoom-in
    # rotate z: 
    # rotate y: left-right
    # rotate x: up-down
    #
    ...
    # proj  = util.projection(x=0.1, n=1.0, f=20.0, orthographic=False)
    # r_mv  = util.translate(0.2, 1., -15.0)
    # r_mv  = np.matmul(r_mv, rotate_z(np.pi))
    # r_mv  = np.matmul(r_mv, util.rotate_y(np.pi*1.1))
    # r_mv  = np.matmul(r_mv, util.rotate_x(np.pi*2.1))
    # r_mvp = np.matmul(proj, r_mv).astype(np.float32)
    # r_mvp = torch.as_tensor(r_mvp, dtype=torch.float32, device='cuda')
    # #
    # # proj = util.projection(x=3., n=-10.0, f=200.0, orthographic=True)
    # # r_mv  = util.translate(0., -1., -0.)
    # # r_mv  = np.matmul(r_mv, rotate_z(np.pi*0.25))
    # # r_mv  = np.matmul(r_mv, util.rotate_y(np.pi*0.25))
    # # r_mv  = np.matmul(r_mv, util.rotate_x(np.pi*0.25))
    # # r_mvp = np.matmul(proj, r_mv).astype(np.float32)
    # # r_mvp = torch.as_tensor(r_mvp, dtype=torch.float32, device='cuda')

    # # # room
    # # # Modelview and projection matrices.
    # # proj  = util.projection(x=.1, n=1.0, f=500.0)
    # # r_mv  = util.translate(.0, 5., -110.)
    # # r_mv  = np.matmul(r_mv, rotate_z(np.pi))
    # # r_mv  = np.matmul(r_mv, util.rotate_y(np.pi*1.2))
    # # r_mv  = np.matmul(r_mv, util.rotate_x(np.pi/100))
    # # r_mvp = np.matmul(proj, r_mv).astype(np.float32)
    # # r_mvp = torch.as_tensor(r_mvp, dtype=torch.float32, device='cuda')
    #
    # cube
    ...
    proj_params = [
        # x, n, f
        (.1, 1.0, 60.0), # (.1, 1.0, 20.0)
        (.1, 1.0, 60.0) # (.1, 1.0, 20.0)
        ]
    translations = [
        # x, y, z
        (0., .5, -20.0), # (0.2, 1., -15.0)
        (0., 1.51, -20.0) # (0.2, 1., -9.0) or -17 for different zoom
        ]
    rotations = [
        #z, y, x
        (np.pi, np.pi*2.1, np.pi*0), # (np.pi, np.pi*1.1, np.pi*2.1)
        (np.pi, np.pi*2, np.pi*0) # (np.pi, np.pi*1, np.pi*0)
        ]
    #
    proj_params = [
        # x, n, f
    ...
    # rotations = [
    #     #z, y, x
    #     (np.pi, np.pi*1.3, np.pi*2),
    #     (np.pi, np.pi*1.2, np.pi*2.2)
    #     ]
    ...
    for resolution in resolutions:
        results[resolution] = {
            ...
            'params': [],
            'footprints': [],
            'map': [], # Thanos, better rename to mapping
            'normal': []
        }
    ...
    for idx, resolution in enumerate(resolutions):
        ...
        for param_idx in range(len(proj_params)):
            ...
            # Samuli suggestion (interpolate post_projection w)
            ...
            valid_mask = w_center != 0 # Thanos: Hmmm.... 1 pxl seems to be False (or more) but there is no background
            ...
            for dy, dx in [(-0.5, -0.5), (-0.5, 0.5), (0.5, 0.5), (0.5, -0.5)]:
                ...
                corner_u = torch.where(valid_mask, corner_u * 1024, torch.full_like(corner_u, 0)) # 1->1024
                corner_v = torch.where(valid_mask, corner_v * 1024, torch.full_like(corner_v, 0)) # 1->1024
            ...
            if param_idx == 0:
                for f in footprints[18,9]:
                    print(f[0].item(), f[1].item())
            else:
                for f in footprints[18,14]:
                    print(f[0].item(), f[1].item())
...
if __name__ == "__main__":
    ...
    print(
        (rendered[128]["footprints"][0][18,9] - rendered[128]["footprints"][1][18,14])[0,1]
        )
    
    # # orthographic
    # print(
    #     (rendered[128]["footprints"][0][33,33] - rendered[128]["footprints"][1][33,30])[0,1]
    #     )
    ...
    # Define resolution
    resolution = (4096, 4096) #(4096, 4096) # 512, 1024

       
# =============================================================================
#     # qc = rendered[128]["footprints"][0][100,50] * resolution[0] / 1024
#     # get 1st key (for testing may be different than 128, so get the 1st whatever it is)
#     qc = rendered[next(iter(rendered))]["footprints"][0][20,30] * resolution[0] / 1024
#     qc = torch.tensor(qc, dtype=torch.float32).cuda()    
#     
#     print(qc)
# 
#     # quad_coords2 = [torch.tensor(qc, dtype=torch.float32, device="cuda") for qc in inp[128]["footprints"]]  # for all views
#     # # print(len(quad_coords2))
#     # # print(quad_coords2[0].shape)
# 
#     # Get pixels inside quad
#     pixel_coords, mask = get_pixels_in_quad(glctx, qc, resolution)
#     
#     print(f"Found {len(pixel_coords)} pixels inside the quad")
#     # print("First few pixel coordinates:", pixel_coords.cpu()[:5]) 
# 
#     # Visualize results
#     visualize_pixels(resolution, qc, pixel_coords, mask)
# =============================================================================


# # %% above is unique, here is for all
# OPTIMIZE !!! (Thanos)
        
    # for view in range(len(rendered[128]["footprints"])):
    #     footprints = rendered[128]["footprints"][view] * resolution[0] / 1024
    #     # Reshape to a list of quads
    ...
    #         if quad_idx % 1000 == 0:  # Progress indicator
    #             print(f"Processing quad {quad_idx}/{len(footprints_tensor)}")
                
    #         # Get pixels inside current quadD
    #         pixel_coords, _ = get_pixels_in_quad(glctx, qc, resolution)
    #         view_pixel_coords.append(pixel_coords)
            

    #     rendered[128]["map"].append(view_pixel_coords)
        
    #     # rendered[128]["map"].append([view_pixel_coords[i:i+128] for i in range(0, len(view_pixel_coords), 128)])
...
# # %% plotting rasterized texels

# # all_texels = []
# # for y in range(128):
# #     for x in range(128):
# #         texels = rendered[128]["map"][0][y, x]
# #         if len(texels) == 0:
# #             continue
# #         else:
# #             all_texels.extend(texels.tolist())  # Append all texels directly
# # all_texels = np.array(all_texels)
# # #
# # # # Create an empty 4096x4096 grid filled with red (e.g., [1, 0, 0] for RGB red)
# # # plane = np.ones((resolution[0], resolution[0], 3))
# # # # Set the background to red
# # # plane[:, :] = [1, 0, 0]  # Red color
# # # # Convert all_texels to a numpy array
# # # all_texels = np.array(all_texels)
# # # # Loop through all texels and set the corresponding coordinates to blue
# # # for texel in all_texels:
# # #     y, x = texel  # Extract x, y coordinates
# # #     if 0 <= x < resolution[0] and 0 <= y < resolution[0]:
# # #         plane[y, x] = [0, 0, 1]  # Blue color
# # # # Plot the result
# # # plt.imshow(plane)
...
# #
# image =  imageio.v2.imread("default.png")[::-1].astype(np.float32) / 255.0 ###########################################.v2
# tex = resize(image, (4096, 4096), mode='reflect', anti_aliasing=True)[None,...]
# tex = np.transpose(tex, (0,3,1,2))
# #
# # Add a new channel for pxls_in_quad_num (extra channel for previous view's values)
# tex = np.concatenate([tex, -np.ones((1, 1, tex.shape[2], tex.shape[3]), dtype=np.float32)], axis=1)

# #
# rendered_latents = [np.zeros((1, 4,128, 128), dtype=np.float32) for _ in range(len(rendered[128]["view"]))]
# #
# for idx, delta in enumerate(rendered[128]["view"]):
#     print(f"Updating delta for view {idx}")
#     # if idx == 1:
#     #     continue
#     for y in range(128):
#         for x in range(128):
#             if (rendered[128]["footprints"][idx][y,x] == 0).all():
#                 rendered_latents[idx][...,y,x] = 0
#             else:
#                 pxl_coords = [rendered[128]["map"][idx][y,x]][0].cpu().numpy() #* 8
#                 pxls_in_quad_num = pxl_coords.shape[0]
#                 last_channel_values = tex[:, -1, pxl_coords[:, 0], pxl_coords[:, 1]]
                
#                 # Check for -1 in last_channel_values (initialization value)
#                 invalid_mask = last_channel_values == -1
#                 if np.any(invalid_mask):
#                     # If there are -1 values, set them to the number of pixels in the quad
#                     last_channel_values[invalid_mask] = pxls_in_quad_num
                
#                 # if idx == 0:
#                 # tex[:, :-1,
#                 #     pxl_coords[:, 0],  # -> y
#                 #     pxl_coords[:, 1]  # -> x
#                 #     ] -= 0/np.sqrt(pxls_in_quad_num)

#                 quad_pxl_values = tex[:, :-1,
#                                     pxl_coords[:,0], # -> y
#                                     pxl_coords[:,1] # -> x
#                                     ]
                      
#                 scale = np.sqrt(last_channel_values)
#                 rendered_latents[idx][..., y, x] = (quad_pxl_values * scale).sum(axis=-1) / pxls_in_quad_num
#                 # rendered_latents[idx][..., y, x] = np.log(rendered_latents[idx][..., y, x])
#                 # #
#                 # l2_norm = np.linalg.norm(np.linalg.norm(last_channel_values, axis=(-1)))
#                 # rendered_latents[idx][..., y, x] = quad_pxl_values.sum(axis=-1) / (l2_norm + 1e-8)
...
#     rgb_image = np.transpose(rendered_latents[idx][0], (1,2,0))
...
#     plt.imshow(tex.transpose(0, 2, 3, 1)[0,:,:,:-2])
#     # plt.colorbar()
#     plt.show()

# # %% the above but with rendering
...
#                     pxl_coords[:, 0],  # -> y
#                     pxl_coords[:, 1]  # -> x
#                     ] -= 0/pxls_in_quad_num #np.sqrt(pxls_in_quad_num)
...
#     texb = np.transpose(tex[0,:-1], (1,2,0))
#     texb = torch.as_tensor(texb, dtype=torch.float32, device='cuda')
#     #
#     x = rendered[128]["params"][idx]["projection"]["x"]
#     n = rendered[128]["params"][idx]["projection"]["n"]
#     f = rendered[128]["params"][idx]["projection"]["f"]
#     tx = rendered[128]["params"][idx]["translation"]["x"]
#     ty = rendered[128]["params"][idx]["translation"]["y"]
#     tz = rendered[128]["params"][idx]["translation"]["z"]
#     rx = rendered[128]["params"][idx]["rotation"]["x"]
#     ry = rendered[128]["params"][idx]["rotation"]["y"]
#     rz = rendered[128]["params"][idx]["rotation"]["z"]
#     proj = util.projection(x=x, n=n, f=f, orthographic=False)
#     r_mv = util.translate(tx, ty, tz)
#     r_mv = np.matmul(r_mv, rotate_z(rz))
#     r_mv = np.matmul(r_mv, util.rotate_y(ry))
...
#     rast_out, rast_out_db = dr.rasterize(glctx, cpos.contiguous(), tri_pos, resolution=resolution)
#     attr_out, attr_out_da = dr.interpolate(uv[None, ...], rast_out, tri_uv, rast_db=rast_out_db, diff_attrs='all')
#     tex_out = dr.texture(texb[None, ...], attr_out, attr_out_da, filter_mode='linear-mipmap-linear')
#     # to remove the background:
#     texw_out = torch.where(rast_out[..., 3:] > 0, tex_out, torch.tensor(1.0).cuda())
#     plt.imshow(texw_out[0].detach().cpu().numpy())
#     plt.title(f'Texture sampling ({resolution[0]}x{resolution[0]})')
...
for idx, delta in enumerate(rendered[128]["view"]):
    ...
    for y in range(128):
        for x in range(128):
            if (rendered[128]["footprints"][idx][y,x] == 0).all():
                ...
                Nor = np.where(
                    tex[0, -1, pxl_coords[:,0], pxl_coords[:,1]] == -1,
                    N,
                    tex[0, -1, pxl_coords[:,0], pxl_coords[:,1]]
                    )
                average_per_pixel = quad_pxl_values #* np.sqrt(Nor) / N
                rendered_latents[idx][...,y,x] =  average_per_pixel.sum(axis=-1) / np.sqrt(N)
                
                if idx == 2 and x == 64 and y == 64:
                    print(rendered_latents[idx][...,y,x])
                if idx == 2 and x == 40 and y == 40:
                    print(rendered_latents[idx][...,y,x])
    ...
    plt.imshow(tex.transpose(0, 2, 3, 1)[0,:,:,:-2]) # channel last and use 3 of the 5 channels for visualization
    # plt.colorbar()
    plt.show()
...
class MYStableDiffusionXLControlNetPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #     for _ in inp[128]["view"]
        #     ]
        # # noise[1] = noise[0]
        # #
        # latents = []
        # for v, n in zip(inp[1024]["view"], noise):
        #     # im = v[...,:3].cpu().numpy().astype(np.float16).transpose(0, 3, 1, 2)  # Adjust shape
        #     im = v.cpu().numpy().astype(np.float16).transpose(0, 3, 1, 2)  # Adjust shape
        ...
        #     # latents.append(self.scheduler.add_noise(latent, n, timesteps[-1:])) # just for testing, thanos
        # #
        # # initialize big_latent and big_latent_temp
        # # initilize with zeros
        # big_latent = torch.zeros([1, 4, 4096, 4096], dtype=torch.float16).to("cuda") # last channel is frequency
        # # add normals channel
        # temp_last_channel = -torch.ones(big_latent.size(0), 1, big_latent.size(2), big_latent.size(3), device=big_latent.device, dtype=torch.float16)
        # big_latent = torch.cat([big_latent, temp_last_channel], dim=1)
       
                        

        # # Random noise, w/o inversion ...
        # latents = []
        # latents = [
        ...
        # # make the frequency channel 1's # ToDo, remove it from here, not relevant during initialization
        # temp_last_channel = torch.ones(big_latent.size(0), 1, big_latent.size(2), big_latent.size(3), device=big_latent.device)
        # big_latent = torch.cat([big_latent, temp_last_channel], dim=1)
        #
        # initialize the latent space
        # # latents = [torch.empty(1, 4, 128, 128, device="cuda") for _ in range(len(inp[128]["view"]))]
        # latents = [torch.randn([1, 4, 128, 128], dtype=torch.float16).to("cuda") for _ in range(len(inp[128]["view"]))]
        #
        # Generate one random tensor
        ...
        # populate the latents from the big_latent
            ...
            for y in range(lt.shape[2]):
                for x in range(lt.shape[2]):
                    if (inp[128]["footprints"][idx][y,x] == 0).all():
                        ...
                        average_per_pixel = quad_pxl_values #/ big_latent[0, -1,
                                                            #             [inp[128]["map"][idx][y,x]][0][:,0], # -> y
                                                            #             [inp[128]["map"][idx][y,x]][0][:,1] # -> x
                                                            #             ]
                        # # this seems to work same as the np.sqrt
                        # l2_norm = torch.norm(average_per_pixel, p=2, dim=-1)
                        # latents[idx][..., y, x] = average_per_pixel.sum(axis=-1) / (l2_norm + 1e-8)
                        #
                        latents[idx][...,y,x] =  average_per_pixel.sum(axis=-1) / np.sqrt(average_per_pixel.shape[-1])
            #
            # # plt.hist(big_latent.view(-1).cpu().numpy(), bins=100)
            # plt.hist(latents[0].view(-1).cpu().numpy(), bins=100)
            # plt.hist(latents[1].view(-1).cpu().numpy(), bins=100)
        
        
        
        
        # rendering (same as averaging)
        # big_latent = self.prepare_latents(
        #     batch_size * num_images_per_prompt,
        #     num_channels_latents,
        #     4096*8,
        ...
        #     r_mvp = torch.as_tensor(r_mvp, dtype=torch.float32, device='cuda')
        #     cpos = torch.matmul(r_mvp, pos.t()).t()[None, ...]
        #     rast_out, rast_out_db = dr.rasterize(glctx, cpos.contiguous(), tri_pos, resolution=(128, 128))
        #     attr_out, attr_out_da = dr.interpolate(uv[None, ...], rast_out, tri_uv, rast_db=rast_out_db, diff_attrs='all')
        #     bl = torch.as_tensor(big_latent.permute(0,2,3,1).contiguous(), dtype=torch.float32, device='cuda')
        #     tex_out = dr.texture(bl, attr_out, attr_out_da, filter_mode='linear-mipmap-linear')
        #     # to remove the background:
        #     latents[idx] = torch.where(rast_out[..., 3:] > 0, tex_out, latents[idx].permute(0, 2, 3, 1))
        #     latents[idx] = latents[idx].permute(0, 3, 1, 2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for idx, (ltnt, depth, _) in enumerate(zip(latents, image, inp[128]["params"])):
                    ...
                    ltnts.append(ltnt)
                    # delta.jj()
                    # print(f"Step {i}, Delta Stats: min={delta[idx].min()}, max={delta[idx].max()}, mean={delta[idx].mean()}")
                    # print(f"Step {i}, ltnt Stats: min={ltnt.min()}, max={ltnt.max()}, mean={ltnt.mean()}")
                    # plt.plot(ltnt[0,0].flatten().cpu().numpy())
                    # plt.show()
                    
                                        
                    # latents[idx] = ltnt
                    
                # # # Reset frequency channel to zeros
                # # # Thanos: outside of the loop, does not need to be repeated (unless if it changes)
                # # big_latent[:,-1,:,:] = 0
                # # # For each view/idx
                # # for idx in range(len(inp[128]["map"])):
                # #     # For each y coordinate
                # #     for y in range(128):  # Assuming 128x128 grid
                # #         # For each x coordinate
                # #         for x in range(128):
                # #             # Skip if no coordinates in this pixel
                # #             if (inp[128]["footprints"][idx][y,x] == 0).all():
                # #                 continue     
                # #             # Get coordinates for this y,x position
                # #             coords = [inp[128]["map"][idx][y,x]][0]  # Extract coordinates
                # #             # Add 1 to frequency channel for each coordinate
                # #             big_latent[0, -1, coords[:,0], coords[:,1]] += 1

                
                # # big_latent_current = big_latent.repeat(len(inp[128]["view"]), 1, 1, 1, 1)
                # # big_latent_current[1] = big_latent_current[0].clone()
                
                
                # # plt.hist(ltnts[1][...,60:,10:100].flatten().cpu().numpy(), bins=100)
                # # plt.show()
                ...
                #     #     continue
                #     print(f"Updating delta for view {idx}")
                #     # map noise_pred to big_latent
                #     for y in range(ltnt.shape[2]):
                #         for x in range(ltnt.shape[2]):
                #             if (inp[128]["footprints"][idx][y,x] == 0).all(): # thanos: x,y re-check it
                #                 continue
                #             else:
                #                 pxl_coords = [inp[128]["map"][idx][y,x]][0]
                #                 pxls_in_quad_num = pxl_coords.shape[0]
                #                 last_channel_values = big_latent[:, -1, pxl_coords[:, 0], pxl_coords[:, 1]]
                #                 # # Check for -1 in last_channel_values (initialization value)
                #                 # invalid_mask = last_channel_values == -1
                ...
                #                 l = ltnts[idx][...,y,x]
                #                 N = pxls_in_quad_num
                #                 L = l.unsqueeze(-1).repeat(1, 1, N)
                #                 # big_latent[:, :-1,
                #                 # # big_latent_current[idx][:, :-1,
                #                 #             pxl_coords[:, 0],  # -> y
                #                 #             pxl_coords[:, 1]  # ->inp[128]["footprints"][idx][y,x] x
                #                 #             ] -= D/np.sqrt(N)
                #                 #
                ...
                #                 #             pxl_coords[:, 1]  # -> x
                #                 #             ] = L
                #                 big_latent_current[idx][:,:-1,pxl_coords[:,0],pxl_coords[:,1]] = L
                #                 big_latent_current[idx][:,-1,pxl_coords[:,0],pxl_coords[:,1]] = inp[128]["normal"][idx][0,y,x].to(torch.float16) # Thanos, fix it to be float16
                # # view selection
                # # Find which view has better normals
                # max_norm_values, max_norm_indices = torch.max(big_latent_current[:, 0, -1], dim=0)
                # # Update the content channels (first 4 channels)
                # for view_idx in range(big_latent_current.shape[0]):
                #     mask = (max_norm_indices == view_idx)
                #     # plt.imshow(mask.cpu().numpy())
                #     # # purple: taken from another view
                #     # # yellow: contribute for other views to take
                #     # plt.show()
                #     big_latent[0, :4, mask] = big_latent_current[view_idx, 0, :4, mask]
                # # Update the frequency channel with the maximum frequencies
                # big_latent[0, -1] = max_norm_values # thanos, this is not needed, to be removed
                # #
                # # averaging using normals (first main channels, then last channel)
                # # main_channels = torch.where(
                # #     # Condition 1: Both have -1 in last channel
                # #     (big_latent_current[0][:, -1:, :, :] == -1) & (big_latent_current[1][:, -1:, :, :] == -1),
                # #     torch.full_like(big_latent_current[0][:, :-1, :, :], -1),  # Return -1 if both are -1
                # #     torch.where(
                # #         # Condition 2: First tensor has -1
                # #         big_latent_current[0][:, -1:, :, :] == -1,
                # #         big_latent_current[1][:, :-1, :, :],  # Use second tensor values
                # #         torch.where(
                # #             # Condition 3: Second tensor has -1
                # #             big_latent_current[1][:, -1:, :, :] == -1,
                # #             big_latent_current[0][:, :-1, :, :],  # Use first tensor values
                # #             # Condition 4: Neither has -1, compute weighted average
                # #             (big_latent_current[0][:, :-1, :, :] * big_latent_current[0][:, -1:, :, :] + 
                # #              big_latent_current[1][:, :-1, :, :] * big_latent_current[1][:, -1:, :, :]) / 
                # #             (big_latent_current[0][:, -1:, :, :] + big_latent_current[1][:, -1:, :, :])
                # #         )))
                # # last_channel = torch.where(
                # #     # Both are -1
                # #     (big_latent_current[0][:, -1:, :, :] == -1) & (big_latent_current[1][:, -1:, :, :] == -1),
                # #     torch.full_like(big_latent_current[0][:, -1:, :, :], -1),
                # #     torch.where(
                # #         # First is -1
                # #         big_latent_current[0][:, -1:, :, :] == -1,
                # #         big_latent_current[1][:, -1:, :, :],
                # #         torch.where(
                # #             # Second is -1
                # #             big_latent_current[1][:, -1:, :, :] == -1,
                # #             big_latent_current[0][:, -1:, :, :],
                # #             # Neither is -1, take average
                # #             (big_latent_current[0][:, -1:, :, :] + big_latent_current[1][:, -1:, :, :]) / 2
                # #         )))
                # # # Concatenate main channels with last channel
                # # big_latent = torch.cat([main_channels, last_channel], dim=1)
                # #
                # #
                # for idx, delta in enumerate(deltas):       
                #     for y in range(ltnt.shape[2]):
                ...
                #             else:
                #                 pxl_coords = [inp[128]["map"][idx][y,x]][0]
                #                 # # Increment the -1 channel for all selected pixels
                #                 # big_latent[:, -1,
                #                 #             pxl_coords[:, 0],  # -> y
                #                 #             pxl_coords[:, 1]  # -> x
                #                 #             ] += 1
                #                 quad_pxl_values = big_latent[:, :-1,
                #                 # quad_pxl_values = big_latent_current[idx][:, :-1,
                #                                     pxl_coords[:,0], # -> y
                #                                     pxl_coords[:,1] # -> x
                #                                     ][0]
                #                 # scale = torch.sqrt(last_channel_values)
                #                 # latents[idx][..., y, x] = (quad_pxl_values * scale).sum(dim=-1) / pxls_in_quad_num    
                #                 # big_latent[:, -1,
                #                 #     pxl_coords[:, 0],
                #                 #     pxl_coords[:, 1]
                #                 #     ] = pxls_in_quad_num
                #                 #
                #                 latents[idx][..., y, x] = quad_pxl_values.mean(dim=-1)
                #                 # average_per_pixel = quad_pxl_values #/ big_latent_current[idx][0, -1,
                #                 #                                     #              pxl_coords[:,0], # -> y
                #                 #                                     #              pxl_coords[:,1] # -> x
                #                 #                                     #              ] 
                #                 # latents[idx][...,y,x] =  average_per_pixel.sum(dim=-1) / np.sqrt(N)#(pxls_in_quad_num)
                

                # for idx, delta in enumerate(deltas):
                #     # works for one..... [Thanos]
                #     if idx == 1:
                #         continue
                #     print(f"Updating delta for view {idx}")
                #     # map noise_pred to big_latent
                ...
                # quad_ids
                quad_ids = -torch.ones([1, 2, 4096, 4096], dtype=torch.float16).to("cuda")
                    ...
                    for y in range(ltnt.shape[2]):
                        for x in range(ltnt.shape[2]):
                            if (inp[128]["footprints"][idx][y,x] == 0).all(): # thanos: x,y re-check it
                                ...
                                average_per_pixel = quad_pxl_values * torch.sqrt(Nor) / N
                                latents[idx][...,y,x] =  average_per_pixel.sum(dim=-1)
                                
                                
                                
                                
                                # latents[idx][...,y,x] =  quad_pxl_values.sum(dim=-1)/N
                                # if first_pass == True:
                                #     lg.debug(pxl_coords, N)
                                #     lg.debug(latents[idx][...,y,x])
                                # first_pass = False
                                
                                
                                
                                if idx==0:
                                    big_latent[:, -1, pxl_coords[:, 0], pxl_coords[:, 1]] = N
                    lg.debug("----------")
                # plt.hist(latents[idx].flatten().cpu().numpy())
                # plt.show()
                
                
                
                tt = big_latent[0,-1:,1100:2000,2900:-100].cpu().numpy()
                
                
                
                
  
        # # rendering the quads from the previous view 
        
        for idx, delta in enumerate(deltas):
            for y in range(ltnt.shape[2]):
                for x in range(ltnt.shape[2]):
                    if (inp[128]["footprints"][idx][y, x] == 0).all():  # Check if all footprints are 0
                        latents[idx][..., y, x] = ltnts[0][..., y, x]
                    else:
                        pxl_coords = [inp[128]["map"][idx][y, x]][0]
        
                        # Apply filtering for `idx != 0` using `quad_ids`
                        if idx != 0:
                            valid_mask = (
                                (quad_ids[0, 0, pxl_coords[:, 0], pxl_coords[:, 1]] == y) &
                                (quad_ids[0, 1, pxl_coords[:, 0], pxl_coords[:, 1]] == x - 5) #&
                                # (quad_ids[0, 1, pxl_coords[:, 0], pxl_coords[:, 1]] <= x - 4)        
                            )
                            pxl_coords = pxl_coords[valid_mask]
        
                        # Skip if no valid coordinates remain
                        if pxl_coords.shape[0] == 0:
                            print("---------")
                        ...
                        # Update quad_ids only for `idx == 0`
                        if idx == 0:
                            quad_ids[:, 0, pxl_coords[:, 0], pxl_coords[:, 1]] = y
                            quad_ids[:, 1, pxl_coords[:, 0], pxl_coords[:, 1]] = x
        ...
        plt.imshow(images[1][0])                    
                ...
                # N = 30*30
                # d =  torch.tensor(.02, dtype=torch.float32)
                # D = d.unsqueeze(-1).repeat(1, 1, N)
                # # data = torch.tensor(1.234, dtype=torch.float32).repeat(1,1,N)
                # data = torch.randn(1, 1, N, dtype=torch.float32)
                # plt.hist(data.flatten().cpu().numpy()), plt.show()
                # data -= D/np.sqrt(N)
                # plt.hist(data.flatten().cpu().numpy()), plt.show()
                # # rendering
                # avg = data *  torch.sqrt(torch.tensor(N, dtype=torch.float32))      / N         
                # final = avg.sum() #/ N 
                # print(final)
                # # #
                # newN = 10*10
                # newdata = data[...,:newN]
                # newavg = newdata *  torch.sqrt(torch.tensor(newN, dtype=torch.float32)) / N              
                # final = avg.sum() #/ N
                # print(final)
                ...
                #     plt.hist(data.flatten().cpu().numpy()), plt.show()
                #     # rendering
                #     avg = data *  torch.sqrt(torch.tensor(N, dtype=torch.float32))               
                #     final = avg.sum() / N 
                #     tt.append(final)
                # tt = np.array(tt)
                # plt.hist(tt)
                ...
                # tri_uv  = torch.as_tensor(tri_uv,  dtype=torch.int32,   device='cuda')
                # glctx = dr.RasterizeCudaContext()
                # for i, lt in enumerate(ltnts):
                #     x = inp[128]["params"][i]["projection"]["x"]
                #     n = inp[128]["params"][i]["projection"]["n"]
                #     f = inp[128]["params"][i]["projection"]["f"]
                #     tx = inp[128]["params"][i]["translation"]["x"]
                #     ty = inp[128]["params"][i]["translation"]["y"]
                #     tz = inp[128]["params"][i]["translation"]["z"]
                #     rx = inp[128]["params"][i]["rotation"]["x"]
                #     ry = inp[128]["params"][i]["rotation"]["y"]
                #     rz = inp[128]["params"][i]["rotation"]["z"]
                #     proj = util.projection(x=x, n=n, f=f, orthographic=False)
                #     r_mv = util.translate(tx, ty, tz)
                #     r_mv = np.matmul(r_mv, rotate_z(rz))
                #     r_mv = np.matmul(r_mv, util.rotate_y(ry))
                ...
                #     rast_out, rast_out_db = dr.rasterize(glctx, cpos.contiguous(), tri_pos, resolution=(128, 128))
                #     attr_out, attr_out_da = dr.interpolate(uv[None, ...], rast_out, tri_uv, rast_db=rast_out_db, diff_attrs='all')
                #     bl = torch.as_tensor(big_latent[:,:-1].permute(0,2,3,1).contiguous(), dtype=torch.float32, device='cuda')
                #     tex_out = dr.texture(bl, attr_out, attr_out_da, filter_mode='linear-mipmap-linear')
                #     # to remove the background:
                #     lt = torch.where(rast_out[..., 3:] > 0, tex_out, lt.permute(0, 2, 3, 1))
                #     latents[i] = lt.permute(0,3,1,2).to(torch.float16) #Thanos: fix need for dtype
                        
                
                # big_latent = big_latent_current[0].clone()
                # plt.hist(latents[1].flatten().cpu().numpy(), bins=100)
                # plt.hist(latents[1][...,60:,10:100].flatten().cpu().numpy(), bins=100)
                # big_latent.jj()
                # plt.show()
                # latents[0].jj()
                # latents[1].jj()
        ...
        # tt =  StableDiffusionXLPipelineOutput(images=images)
        
        # fig, axes = plt.subplots(1, 2, figsize=(10, 5))
        # axes[0].imshow(tt[0][0][0])
        # axes[0].axis('off')
        # axes[1].imshow(tt[0][1][0])
        # axes[1].axis('off')
        # plt.tight_layout()
        # plt.show()
        ...
        # print("BP")
                ...
                # if (i == 0 or i == 29):
                #     plt.hist(big_latent_current[0][:,:-1].view(-1).cpu().numpy(), bins=100)
                #     plt.title("Distribution of big_latent_current[0]")
                #     plt.show()
                #     plt.hist(big_latent_current[1][:,:-1].view(-1).cpu().numpy(), bins=100)
                #     plt.title("Distribution of big_latent_current[1]")
                #     plt.show()
                #     plt.hist(big_latent[:,:-1].view(-1).cpu().numpy(), bins=100)
                #     plt.title("Distribution of big_latent_current[0]")
                #     plt.show()
                    
                # if (i == 0 or i == 29):
                #     plt.hist(latents[0].view(-1).cpu().numpy(), bins=100)
                #     plt.title("Distribution of latents[0]")
                #     plt.show()
                #     plt.hist(latents[1][:,:-1].view(-1).cpu().numpy(), bins=100)
                #     plt.title("Distribution of latents[1]")
                #     plt.show()
...
# =============================================================================
#                                 # print(np.round(ltnt[...,y,x].tolist()[0][0] - latents[idx][...,y,x].tolist()[0][0], 2),
#                                 # np.round(ltnt[...,y,x].tolist()[0][1] - latents[idx][...,y,x].tolist()[0][1], 2),
#                                 # np.round(ltnt[...,y,x].tolist()[0][2] - latents[idx][...,y,x].tolist()[0][2], 2),
#                                 # np.round(ltnt[...,y,x].tolist()[0][3] - latents[idx][...,y,x].tolist()[0][3], 2))
#                                 # print("-"*20)
#                                 #
#                                 #
#                                 # print("##########", ltnt[..., y, x].mean().item(), latents[idx][...,y,x].mean().item())
#                                 #
#                                 #
# =============================================================================
